...
const NuevoGastoModal = ({ incomingBill, reload }) => {
  const [projects, setProjects] = useState([]);
  const [customer, setCustomer] = useState([]);
  const [supplier, setSupplier] = useState([]);
  const [show, setShow] = useState(false);

  const newIncomingBillSchema = Yup.object().shape({
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      let response;
      if (incomingBill != undefined) {
        const url = `${BACKEND}/api/incomingbill/${incomingBill.id}`;
        ...
        const url = `${BACKEND}/api/incomingbill`;
        response = await fetch(url, {
          method: "POST",
          body: JSON.stringify(values),
          headers: {
                    ...
                    <div className="grid grid-cols-2 md:grid-rows-8">
                      ...
                      <div className="pl-3 pt-1.5">
                        ...
                        >
                          <option
                            value=""
                            label="Seleccione la modalidad de la operación"
                          ></option>
